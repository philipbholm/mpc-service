...
echo "alias build='docker buildx --builder repro build -t server --platform linux/arm64 --build-arg SOURCE_DATE_EPOCH=0 --output type=docker,dest=server.tar,rewrite-timestamp=true src/enclave && docker load -i server.tar && docker run --rm --platform linux/arm64 -v /var/run/docker.sock:/var/run/docker.sock builder'" >> .bashrc
echo "alias nocache='docker buildx --builder repro build -t server --platform linux/arm64 --no-cache --build-arg SOURCE_DATE_EPOCH=0 --output type=docker,dest=server.tar,rewrite-timestamp=true src/enclave && docker load -i server.tar && docker run --rm --platform linux/arm64 -v /var/run/docker.sock:/var/run/docker.sock builder'" >> .bashrc
